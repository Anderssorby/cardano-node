...
runTxBuildRaw (AnyCardanoEra era) txins txouts mLowerBound
              ...
              plutusScriptBundles mParamsFile
              metadataFiles mUpdatePropFile
              (TxBodyFile fpath) = do
    validatedCertificates <- validateTxCertificates era certFiles
    -- Spending Scripts
    ...
    txBodyContent <-
      TxBodyContent
        ...
        <*> validateTxWitnessPPData
              era mParamsFile -- Should be separated from plutus script bundles
                  (zip (map plutusScriptType plutusScriptBundles) (map plutusScriptDatum plutusScriptBundles))
    txBody <-
...
validateTxWitnessPPData :: forall era. CardanoEra era
                        -> Maybe ProtocolParamsFile
                        -> [(PlutusScriptType, Maybe Datum)]
                        -> ExceptT ShelleyTxCmdError IO (TxWitnessPPData era)
validateTxWitnessPPData _ _ [] = return TxWitnessPPDataNone
validateTxWitnessPPData era mPParams scriptsDatums =
  case witnessPPDataSupportedInEra era of
    Just _supported ->
        case mPParams of
          Nothing -> left ShelleyTxCmdPParamsNotSpecifiedPlutusScript
          Just _pParamsFile -> TxWitnessPPData (panic "") <$> toApiTypes scriptsDatums
          -- TODO: WE need to generate the REdeemer Datum Map here using generateMintingRedeemer etc..
 ...
 where
   toApiTypes :: [(PlutusScriptType, Maybe Datum)] -> ExceptT ShelleyTxCmdError IO [(PlutusScriptPurpose era, Maybe ScriptDatum)]
   toApiTypes lis  =  map (\(pScriptType, mDatum) -> (toPlutusScriptPurpose psType,toScriptDatum mDatum)) lis
